...
export class Migration20230926083158 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "order" ("id" uuid not null, "user_id" varchar(255) not null, "cart_id" varchar(255) not null, "items" jsonb not null, "payment" jsonb not null, "delivery" jsonb not null, "comments" varchar(255) not null, "status" varchar(255) not null, "total" int not null, constraint "order_pkey" primary key ("id"));');
    ...
    this.addSql('create table "user" ("id" uuid not null, "name" varchar(255) not null, constraint "user_pkey" primary key ("id"));');
